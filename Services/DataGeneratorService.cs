...
public class DataGeneratorService : IDataGeneratorService
{
    ...
    private readonly Dictionary<string, string[]> _genres = new()
    {
        ...
        ["ru-RU"] = new[] { "Эмбиент", "Прогрессив Рок", "Дрим Поп", "Диско Фанк", "Нео Соул", "Электро Хаус", "Альт Кантри", "Инди Поп" }
    };
    ...
    {
        // Combine base seed with index for reproducible per-song generation
        ...
        // Artist: mix of band names and personal names
        string artist;
        if (songRandom.Next(2) == 0)
        {
            artist = $"{words[songRandom.Next(words.Length)]} {_artistTypes[lang][songRandom.Next(_artistTypes[lang].Length)]}";
        }
        else
        {
        ...
        // Generate likes using a separate seeded random
    ...
    {
        // MAD (Multiply-Add-Divide) operation for combining seeds
    ...
    private int GenerateLikes(double averageLikes, Random random)
    {
        ...
        // Probabilistic approach: use fractional part as probability
        var extraLike = random.NextDouble() < fractional ? 1 : 0;
        
        return floor + extraLike;
    }
}
